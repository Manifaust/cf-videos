...
resource "google_service_account" "node" {
  account_id   = "${var.prefix}-node"
  display_name = "${var.prefix} node"
}
...
sed -i -e 's/^\(service_account:\).*\(#.*\)/\1 ${google_service_account.node.email} \2/' "$1"

# Generic updates
random_key=$$(hexdump -n 16 -e '4/4 "%08X" 1 "\n"' /dev/urandom)

sed -i -e 's/^\(internal_ip:\).*\(#.*\)/\1 ${var.subnet_ip_prefix}.252 \2/' "$1"
sed -i -e 's/^\(deployments_network:\).*\(#.*\)/\1 ${var.prefix}-cfcr-network \2/' "$1"
